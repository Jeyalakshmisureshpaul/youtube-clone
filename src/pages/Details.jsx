    ...
    return (
        ...
        <div className="details">
                   ...
                   <div className="card p-5">
                       
                       {
                           idvideo.map((val,ind)=>{
                               return(
                                   <>
                <ReactPlayer  url={val.channelvideo}  width='100%'  height='450px'  controls /> 
         <div className="video-det p-3">
          ...
          <div className="heart">
              {
                  checklikes ? (
                      <>
           <i class="fa fa-heart m-1" aria-hidden="true" ></i> <span className="pl-1 pr-5">{val.likes}</span>
                      </>
                  ) :(
                      <>
             <i class="fa fa-heart-o m-1" aria-hidden="true" onClick={()=>updateLike(val._id)}></i> <span className="pl-1 pr-5">{val.likes}</span>
                      </>
                  )
              }
          ...
          <div className="heart">
          <i class="fa fa-eye m-1" aria-hidden="true"></i> <span className="pl-1 pr-1">{val.views}</span>
          </div>
         ...
         <button className="btn btn-danger" onClick={()=>updateSub(val.channel)} disabled={checksubb}>Subscriber Now</button>
         </div>
         <br />
         <p>{val.title}</p>
         <p className="tag">Tags: <span >{val.tags}</span> </p>
               ...
               <div className="col-md-4 col-12 mb-3 right-side">
                  {
                      data.map((val,ind)=>{
                          return(
                            ...
                            <div className="card p-3 mb-3"  onClick={()=>redirectUrl(val._id)}>
                                <div className="recv ">
                                    <img src={val.channelthumb} alt={val.channelthumb} className="recimg" />
                                    <div className="pl-3">
                                        <p>{val.channel}  </p>
                                        <p>{val.title}</p>
                                    </div>
                   ...
                   {
                   showAllVideo ?(<>
                   <button className="btn btn-danger pt-2" disabled={ showAllVideo} >NO More Data</button>
                   </>) : <button className="btn btn-danger pt-2" onClick={getLoad}>Load More</button>
               }
